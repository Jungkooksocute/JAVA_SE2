...
![image-20241126154549811](反射&动态代理.assets/image-20241126154549811.png)
...
//一般更多的是当做参数进行传递，比如之前写同步代码块的时候写的
...
//小结：上面这三种方式获取的对象都是一样的，都可认为是相等的，具体应用时选哪个应该根据实际情况
```

## 1.4 字节码文件和字节码文件对象

![image-20241126153539185](反射&动态代理.assets/image-20241126153539185.png)

### 获取 包名+类名：

![image-20241126153645611](反射&动态代理.assets/image-20241126153645611.png)
...
| 方法名                                                       | 说明                                                         |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| Constructor<?>[] getConstructors()                           | 获得所有**公共**的构造（只能public修饰）                     |
| Constructor<?>[] getDeclaredConstructors()                   | 获得所有的构造（包含private修饰）//declared在英语里是权限的意思 |
| Constructor<T> getConstructor(Class<?>... parameterTypes)    | 获取指定构造（只能public修饰）                               |
| Constructor<T> getDeclaredConstructor(Class<?>... parameterTypes) | 获取指定构造（包含private修饰）                              |

获取单个构造方法调用上面的两个方法时，括号里面要写和构造方法一样类型对应的字节码对象。
...
| 方法名                                                       | 说明                                                         |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| Method[] getMethods()                                        | 返回所有成员方法对象的数组（只能拿public的）//父类中的所有公共方法也能获取 |
| Method[] getDeclaredMethods()                                | 返回所有成员方法对象的数组，存在就能拿到//父类中的方法不能获取，只能获取本类里的方法 |
| Method getMethod(String name, Class<?>... parameterTypes)    | 返回单个成员方法对象（只能拿public的）//第2个形参也是形参对应的class类型  eg:int.class |
| Method getDeclaredMethod(String name, Class<?>... parameterTypes) | 返回单个成员方法对象，存在就能拿到                           |

#### 方法.getModifiers()获取方法对应的权限修饰符  //返回值是这个修饰符对应的int类型的数据

### 方法.getParameters();获取形参的对象，返回一个形参对象的数组 Parameter[]

### 方法.getExceptionTypes() 获取方法可能会抛出的所有异常， 返回一个Class[]数组

### 关于Invoke:

如果方法的返回值是void,那么就不用管，按照他自己生成的那个Object就行

##### setAccessible(true)//暂时取消权限修饰符带来的影响

![image-20241126161349525](反射&动态代理.assets/image-20241126161349525.png)
...
![image-20241126162118911](反射&动态代理.assets/image-20241126162118911.png)

![image-20241126162330519](反射&动态代理.assets/image-20241126162330519.png)
...
切记一点：代理可以增强或者拦截的方法都在接口中，接口需要写在newProxyInstance的第二个参数里。//proxy在英语中是	代理	的意思

![image-20241126162504805](反射&动态代理.assets/image-20241126162504805.png)



代理类的静态方法的返回值 表示创建代理的那个对象 可以写接口那个名字
    ...
    public static Star createProxy(BigStar bigStar){
       /* java.lang.reflect.Proxy类：提供了为对象产生代理对象的方法：
        ...
        Star star = (Star) Proxy.newProxyInstance(
         ...
         //获得把这个ProxyUtil.class加载到内存的的加载器
